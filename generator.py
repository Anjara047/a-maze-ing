...
def which_number(y: int, x: int, output: str) -> int:
	...
	if coord_x > 9 and coord_y > 7:
		if y == (y_org - 3):
			if (x == (x_org - 3)) or  (x == (x_org + 1)) or  (x == (x_org + 2)) or  (x == (x_org + 3)):
		...
		if y == (y_org - 1):
			if (x == (x_org - 4)) or  (x == (x_org + 2)):
				wall_range = wall_range.intersection(on['west'])
			if (x == (x_org + 1)) or (x == x_org) or(x == (x_org + 2)) or (x == (x_org - 1)) or (x == (x_org - 2)):
				if x != x_org:
					wall_range = wall_range.intersection(on['south'])
				else:
					wall_range = wall_range - on['south']
				if (x == (x_org + 1)) or (x == x_org) or (x == (x_org - 1)) or (x == (x_org - 2)):
		...
		if y == (y_org):
			if (x == (x_org - 3)) or (x == (x_org - 2)) or (x == (x_org - 1)) or  (x == (x_org + 1)) or  (x == (x_org + 2)) or  (x == (x_org + 3)):
				return 15

		if y == (y_org + 1):
			if (x == (x_org - 2)) or  (x == x_org) or (x == (x_org + 2)) or (x == (x_org + 3)):
		...
		if y == (y_org + 2):
			if (x == (x_org - 2)) or  (x == (x_org)) or  (x == (x_org + 1)) or  (x == (x_org + 2)):
				wall_range = wall_range.intersection(on['west'])

		if y == (y_org + 2):
			if (x == (x_org - 1)) or  (x == (x_org + 1)) or  (x == (x_org + 2)) or  (x == (x_org + 3)):
				return 15
	if y == 0:
		wall_range = wall_range.intersection(on['north'])
	if x == 0:
